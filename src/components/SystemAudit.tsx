...
const SystemAudit: React.FC<SystemAuditProps> = ({ logs, onClear, onAudit, isAuditing }) => {
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        ...
        <div className="flex gap-4">
          <button 
            onClick={onAudit}
            disabled={isAuditing}
            className={`h-10 px-6 border border-cyan-500/20 text-cyan-500 text-[10px] font-black uppercase flex items-center gap-3 transition-all ${isAuditing ? 'opacity-50 cursor-not-allowed' : 'hover:bg-cyan-500/10'}`}
          >
            <Cpu className={`w-4 h-4 ${isAuditing ? 'animate-spin' : ''}`} /> {isAuditing ? 'Auditing_Neural_Nodes...' : 'Perform_System_Audit'}
          </button>
          <button 
            onClick={onClear}
