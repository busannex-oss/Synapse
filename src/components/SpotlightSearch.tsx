...
const SpotlightSearch: React.FC<SpotlightSearchProps> = ({ isOpen, onClose, masterDb, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[15vh] bg-black/10 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div 
        className="w-full max-w-2xl bg-white/90 glass rounded-[2.5rem] synapse-shadow border border-white/60 overflow-hidden animate-in slide-in-from-top-4 duration-300"
            ...
            <div className="space-y-2">
              ...
              {results.map(lead => (
                <div 
                  ...
                  className="flex items-center justify-between p-4 rounded-3xl hover:bg-cyan-600 hover:text-white group transition-all cursor-pointer"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-gray-100 group-hover:bg-cyan-500 flex items-center justify-center transition-colors">
